...
def processar_folha_ponto(arquivo_carregado):
    ...
    atraso_entrada = np.maximum((ts_entrada - esperado_entrada).fillna(zero_delta), zero_delta)
    saida_ant_almoco = np.maximum((esperado_saida_almoco - ts_saida_almoco).fillna(zero_delta), zero_delta)
    atraso_volta_almoco = np.maximum((ts_volta_almoco - esperado_volta_almoco).fillna(zero_delta), zero_delta)
    saida_ant_casa = np.maximum((esperado_saida_casa - ts_saida_casa).fillna(zero_delta), zero_delta)
    almoco_real_delta_semana = (ts_volta_almoco - ts_saida_almoco).fillna(zero_delta)
    almoco_excedido = np.maximum((almoco_real_delta_semana - almoco_esperado_delta).fillna(zero_delta), zero_delta)

    faltante_semana = atraso_entrada + saida_ant_almoco + atraso_volta_almoco + saida_ant_casa + almoco_excedido
    extra_semana = np.maximum((ts_saida_casa - esperado_saida_casa).fillna(zero_delta), zero_delta)
    ...
    duracao_manha = np.maximum((ts_saida_almoco - ts_entrada).fillna(zero_delta), zero_delta)
    duracao_tarde = np.maximum((ts_saida_casa - ts_volta_almoco).fillna(zero_delta), zero_delta)
    # Se só bateu entrada e saída (sem almoço), calcula a jornada direta
    jornada_direta = np.maximum((ts_saida_casa - ts_entrada).fillna(zero_delta), zero_delta)
